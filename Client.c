...
{ 
	...
	printf("5) To search and view a record from file by date or name: Search criteria(Name/Date) \"Person's Name(FirstName LastName)/ Record's Date\"\n");
	...
	do
	{
		...
		else if(result = strstr(message, "help")){
			printf("Please Enter A Command\n");
			printf("----------------------------------------------------------------\n");
			printf("1) To submit the new member list: Addmember member_name, date(YYYY-MM-DD), gender, recommender \n");
			printf("2) To check status of the file: Check_status\n");
